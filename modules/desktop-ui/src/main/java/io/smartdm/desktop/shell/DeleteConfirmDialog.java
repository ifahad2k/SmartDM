...
public final class DeleteConfirmDialog extends GlassmorphicDialog {
    ...
    public DeleteConfirmDialog(Stage owner, int count) {
        super(owner, "SmartDM — Delete Downloads");

        Label headerTitle = new Label("Delete Downloads");
        headerTitle.getStyleClass().add("dt");
        Label headerSub = new Label("How would you like to delete " + count + " selected items?");
        headerSub.getStyleClass().add("ds");
        VBox head = new VBox(4, headerTitle, headerSub);

        Label desc = new Label("• Soft Delete: Removes them from the list only.\n• Permanent Delete: Removes them from the list and deletes files on disk.");
        desc.getStyleClass().add("ds");
        desc.setStyle("-fx-text-fill: #E0E2EA; -fx-padding: 10 0;");
