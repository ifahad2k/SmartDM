...
public class SqlCipherScheduleRepository implements ScheduleRepository {
    ...
    public void save(Schedule schedule) {
        String sql = "INSERT INTO schedule (id, name, start_time_iso, end_time_iso, days_of_week, active, missed_trigger_policy, last_run_time, timezone_id, queue_id) " +
                     "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) " +
                     ...
                     "missed_trigger_policy=excluded.missed_trigger_policy, last_run_time=excluded.last_run_time, timezone_id=excluded.timezone_id, queue_id=excluded.queue_id";
        ...
        try (Connection conn = dbManager.getConnection();
            ...
            stmt.setString(10, schedule.getQueueId());
    ...
    private Schedule mapRow(ResultSet rs) throws SQLException {
        ...
        String queueId = "main-queue";
        try { 
            String val = rs.getString("queue_id"); 
            if (val != null && !val.isBlank()) queueId = val; 
        } catch (SQLException e) { /* migration backward compatibility */ }
    ...
    public List<io.smartdm.domain.ScheduleExecution> findExecutionsByScheduleId(String scheduleId) {
        List<io.smartdm.domain.ScheduleExecution> list = new ArrayList<>();
        String sql = "SELECT * FROM schedule_execution WHERE schedule_id = ? ORDER BY execution_time_millis ASC";
        try (Connection conn = dbManager.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    private io.smartdm.domain.ScheduleExecution mapExecutionRow(ResultSet rs) throws SQLException {
        return new io.smartdm.domain.ScheduleExecution(
            rs.getString("id"),
